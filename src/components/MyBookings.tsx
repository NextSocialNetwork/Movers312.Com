...
export default function MyBookings({ bookings, onRefresh, onSelectBooking }: MyBookingsProps) {
  ...
  if (bookings.length === 0) {
    return (
      <div className="bg-white border-2 border-slate-200 rounded-none p-8 text-center space-y-4">
  ...
  return (
    <div className="space-y-4" id="my_bookings_list_container">
      <div className="flex justify-between items-center font-mono">
        <h4 className="text-xs font-bold text-[#0f172a] uppercase tracking-widest flex items-center gap-2">
          <ClipboardList className="w-4 h-4 text-[#2563eb]" />
          Estimates &amp; Booking Status ({bookings.length})
        </h4>
        <span className="text-[10px] font-bold text-brand-gold bg-[#0f172a] border-2 border-slate-900 px-3 py-1 rounded-none uppercase tracking-wider animate-none">
        ...
        {bookings.map((booking) => {
          ...
          return (
            <div 
              ...
              className="td-card cursor-pointer group flex flex-col justify-between bg-white border-2 border-slate-200 hover:border-slate-400 transition-colors"
            >
              {/* Header */}
              <div className="p-4 bg-slate-50 border-b-2 border-slate-200 flex justify-between items-center">
                <div>
                  <span className="text-[10px] bg-slate-200 text-slate-800 border border-slate-300 px-2.5 py-1 rounded-none font-bold">
                    {booking.id}
                  </span>
                  <span className="text-[9px] text-[#2563eb] font-bold uppercase ml-2">
                    {new Date(booking.createdAt).toLocaleDateString()}
                  </span>
                </div>
                
                <span className="flex items-center gap-1 text-[9px] font-bold text-slate-700 bg-slate-100 border border-slate-200 px-2 py-1 uppercase rounded-none tracking-wider">
              ...
              <div className="p-4 space-y-3 text-left">
                <div className="flex justify-between items-start">
                  <div>
                    <h5 className="font-black text-slate-900 text-xs uppercase block tracking-wider">{booking.fullName}</h5>
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-[8px] uppercase font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-1 py-0.5 inline-block mt-1">Incl. $100 Deposit</span>
                  </div>
                </div>
                ...
                <div className="bg-amber-50 rounded-none p-3 border-2 border-amber-200 border-l-4 border-amber-500 flex items-start gap-2 text-[10px] text-amber-800 uppercase">
              ...
              {/* Action feet */}
              <div className="px-4 py-3 bg-slate-50 border-t-2 border-slate-200 flex justify-between items-center text-xs">
                <a 
                ...
                <button
                  ...
                  className="font-bold text-red-500 hover:text-red-700 uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1 p-1 hover:bg-red-50"
